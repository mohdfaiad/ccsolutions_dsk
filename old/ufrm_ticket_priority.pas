...
type
  Tfrm_ticket_priority = class(Tfrm_form_default)
    ...
    memtkp_cod: TStringField;
    memtkp_id: TLongWordField;
    memtkp_name: TStringField;
    memtkp_status: TShortintField;
    memtkp_deleted_at: TDateTimeField;
    memtkp_dt_registration: TDateTimeField;
    cxGrid_1DBTableView1tkp_id: TcxGridDBColumn;
    cxGrid_1DBTableView1tkp_name: TcxGridDBColumn;
    cxGrid_1DBTableView1tkp_status: TcxGridDBColumn;
...
procedure Tfrm_ticket_priority.Action_saveExecute(Sender: TObject);
var
    ...
    dsEdit:
      try
        try
          ...
          strproc_update.StoredProcName := 'proc_ticket_priority_update';
          strproc_update.Prepare;

          strproc_update.ParamByName('p_ctr_token').AsString      := Tconnection.ctr_token;
          strproc_update.ParamByName('p_tkp_cod').AsString        := memtkp_cod.AsString;
          strproc_update.ParamByName('p_tkp_name').AsString       := dbedt_name.Text;
          strproc_update.ParamByName('p_tkp_status').AsShortInt   := dbchk_status.Checked.ToInteger;
      ...
      try
        try
          if Application.MessageBox('Ao Salvar as alterações, as informações antigas não poderão ser recuperadas!', 'Deseja Salvar as Alterações?', MB_YESNO + MB_ICONINFORMATION + MB_DEFBUTTON2) = IDYES then begin
            ...
            strproc_create.StoredProcName := 'proc_ticket_priority_create';
            strproc_create.Prepare;

            strproc_create.ParamByName('p_ctr_token').AsString    := Tconnection.ctr_token;
            strproc_create.ParamByName('p_tkp_name').AsString     := dbedt_name.Text;
            strproc_create.ParamByName('p_tkp_status').AsShortInt := dbchk_status.Checked.ToInteger;
...
procedure Tfrm_ticket_priority.afterInsert;
begin
  ShowMessage('Registro Iserido com Sucesso');
  cxTabSheet_3.Show;
  Trest_ticket_priority.GetTicketPriority(mem);
  ds.DataSet.Last;
end;

procedure Tfrm_ticket_priority.afterUpdate;
begin
...
procedure Tfrm_ticket_priority.FormCreate(Sender: TObject);
begin
  inherited;
  Trest_ticket_priority.GetTicketPriority(mem);
end;
