...
begin
  ...
  if (AViewInfo <> nil) and
    ((Sender as TcxGridDBTableView).DataController.DataSet.Active) then
  begin
    IndiceCampo := (Sender as TcxGridDBTableView).GetColumnByFieldName
      ('pco_status').Index;
...
begin
  inherited;
  if Application.MessageBox('Deseja cancelar essa requisição?', 'REQUISIÇÃO',
    MB_YESNO + MB_ICONQUESTION) = mrYes then
  begin
    motCancel := UpperCase(InputBox('Cancelamento',
      'Qual o motivo do cancelamento? (mínimo 20 caracteres)', motCancel));
    ...
    begin
      ...
      Application.MessageBox('Motivo do cancelamento menor que 20 caracteres!',
        'REQUISIÇÃO', MB_OK + MB_ICONWARNING);
  end;
end;
...
begin
  inherited;
  if Application.MessageBox('Deseja liberar essa requisição?', 'REQUISIÇÃO',
    MB_YESNO + MB_ICONQUESTION) = mrYes then
  begin
    motLib := UpperCase(InputBox('Liberação',
      'Informe uma observação da liberação! (Não obrigatório)', motLib));
    qry.Edit;
...
begin
  ...
  begin
    ...
    if Fields[0].AsInteger > 0 then
      Application.MessageBox(PWideChar('Existe(m) ' + Fields[0].AsString +
        ' requisição(ões) vencida(s)'), 'AVISO', MB_OK + MB_ICONWARNING);
  end;
end;
...
begin
  inherited;
  if (qrypco_status.OldValue <> 'A') and
    ((qrypco_status.Value <> 'A') or (qrypco_status.Value = '')) then
  begin
    Application.MessageBox
      ('Só é permitido alterar uma requisição que esteja em aberto!',
      'Requisição', MB_ICONINFORMATION + MB_OK);
    qry.CancelUpdates;
    Exit;
  end;
end;
